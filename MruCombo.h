...
public:
   ...
   BOOL AddToList(LPCTSTR pstrDocName = NULL)
   {
      ...
      if( ::_tcslen(pstrDocName) == 0 ) return FALSE; // No string
   ...
   BOOL RemoveFromList(LPCTSTR pstrDocName = NULL)
   {
      // Grab text from combo edit control if not otherwise specified
      TCHAR szDocName[t_cchItemLen];
      if( pstrDocName == NULL ) {
   ...
   BOOL ReadFromRegistry(LPCTSTR pstrRegKey, LPCTSTR pstrName)
   {
      ...
      for( DWORD nItem = m_nMaxEntries; nItem > 0; nItem-- ) {
         ...
         if( lRet == ERROR_SUCCESS ) {
            ...
            if( ::_tcslen(szRetString) > 0 ) m_arrDocs.Add(de);
         }
      }
      rk.Close();
      rkParent.Close();
  ...
  END_MSG_MAP()
   ...
   void DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
   {
      ...
      TCHAR szItem[256] = { 0 };
      ATLASSERT(GetLBTextLen(lpDrawItemStruct->itemID)<256);
      GetLBText(lpDrawItemStruct->itemID, szItem);
      dc.SetBkMode(TRANSPARENT);
      dc.SetTextColor(::GetSysColor(bSelected ? COLOR_HIGHLIGHTTEXT : COLOR_WINDOWTEXT));
      dc.DrawText(szItem, -1, &rc, DT_SINGLELINE | DT_NOPREFIX | DT_VCENTER | DT_PATH_ELLIPSIS);
   }
};
